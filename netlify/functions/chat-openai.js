...
function searchBusinesses(query, limit, cityHint) {
  ...
  // UK cities/regions to detect location intent in query
  var ukCities = ['london','birmingham','manchester','leeds','bristol','sheffield','liverpool','nottingham','coventry','leicester','edinburgh','glasgow','cardiff','belfast','newcastle','brighton','reading','oxford','cambridge','southampton','portsmouth','wolverhampton','bradford','derby','exeter','hull','milton keynes','luton','peterborough','stoke','sunderland','basildon','laindon','essex','suffolk','kent','surrey','hertfordshire','norwich','ipswich','colchester','chelmsford'];

  // Detect location from query
  var queryLoc = null;
  var queryWords = query.toLowerCase();
  ukCities.forEach(function(city) {
    if (queryWords.indexOf(city) !== -1) queryLoc = city;
  });
  // Also use cityHint (user's GPS city) if no location in query
  ...
  // Step 1: Search near user's city first
  if (nearbyCity) {
    var nearbyResults = scoreBusinesses(AUNTIE_TOBI_DIRECTORY, nearbyCity).slice(0, limit);
    if (nearbyResults.length > 0) {
      return { results: nearbyResults, scope: 'local', city: nearbyCity };
    }
  }
...
exports.handler = async function(event) {
  ...
  try {
    ...
    let businessContext = '';
    if (isBusinessSearch) {
      // Get user city from request body if passed
      const userCity = body.userCity || '';
      const searchResult = searchBusinesses(lastMessage, 6, userCity);
      const bizResults = searchResult.results;
      const scope = searchResult.scope;
      const city = searchResult.city;

      if (bizResults.length > 0) {
        businessContext = formatBusinessContext(bizResults, scope, city);
      } else {
        // No results at all — try service-only fallback
        const serviceWords = lastMessage.toLowerCase().replace(/[^a-z0-9\s]/g,'').split(/\s+/).filter(w => w.length > 3 && !['find','near','best','good','looking','where'].includes(w));
        if (serviceWords.length > 0) {
          const fallback = searchBusinesses(serviceWords.join(' '), 6, '');
          if (fallback.results.length > 0) {
            businessContext = formatBusinessContext(fallback.results, 'uk', null);
          }
        }
      }
    }
