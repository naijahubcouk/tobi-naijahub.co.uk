// Auntie Tobi — OpenRouter/GPT-4o mini chat function
'use strict';
...
exports.handler = async function(event) {
  ...
  try {
    ...
    var lastMessage = body.messages && body.messages.length > 0
      ? body.messages[body.messages.length - 1].content || ''
      : '';
    var recentMessages = body.messages ? body.messages.slice(-4) : [];

    var isBusinessSearch = /find|looking for|where can i|recommend|near me|hair|makeup|restaurant|food|shop|salon|church|accountant|solicitor|lawyer|doctor|dentist|photographer|fashion|clothing|tailor|business|caterer|cake|gele|wig|stylist|barber|beautician|lash|nail|tutor|cleaner|childminder|dj|travel agent|money transfer|grocery|groceries|foodstore|butcher|pastor|event planner/i.test(lastMessage);

    var businessContext = '';
    if (isBusinessSearch) {
      var userCity = body.userCity || '';
      var searchResult = searchBusinesses(lastMessage, 6, userCity);
      var bizResults = searchResult.results;
      var scope = searchResult.scope;
      var city = searchResult.city;

      if (bizResults.length > 0) {
        businessContext = formatBusinessContext(bizResults, scope, city);
      } else {
        var serviceWords = lastMessage.toLowerCase().replace(/[^a-z0-9\s]/g, '').split(/\s+/).filter(function(w) { return w.length > 3 && ['find','near','best','good','looking','where'].indexOf(w) === -1; });
        if (serviceWords.length > 0) {
          var fallback = searchBusinesses(serviceWords[0], 6, userCity);
          if (fallback.results.length > 0) {
            businessContext = formatBusinessContext(fallback.results, fallback.scope, fallback.city);
          }
        }
      }
    }
    ...
    if (!reply) {
      console.log('OpenRouter error:', JSON.stringify(result).substring(0, 300));
